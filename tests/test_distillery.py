...
def test_oralhistories_add_publish_one_transcript_2d4ja(
    ...
    # DELETE S3 OBJECTS
    s3_response = s3_client.list_objects_v2(
        Bucket=config("ORALHISTORIES_BUCKET"), Prefix=test_id
    )
    print("🐞 s3_client.list_objects_v2", s3_response)
    if s3_response.get("Contents"):
        s3_keys = [{"Key": s3_object["Key"]} for s3_object in s3_response["Contents"]]
    ...
    # wait for HTML to be generated by GitHub Actions
    attempts = 3  # give it 90 seconds
    while True:
        git_repo.remotes.origin.pull()
        if os.path.exists(
            f"{tmp_oralhistories}/transcripts/{item_component_id}/{item_component_id}.html"
        ):
            break
        elif attempts > 0:
            time.sleep(30)
    ...
    assert page.title() == f"Item {test_id}"
